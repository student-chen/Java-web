...
public class GuestHistoryController extends MultiActionController {
	...
	public void queryCheckinOrderInfo(HttpServletRequest request,
		...
		if ((String) request.getParameter("showAllAction").trim() != null) {
			showAllAction = (String) request.getParameter("showAllAction")
					.trim();
			...
			if (showAllAction.equals("0")) { // show single result
				...
				if ((String) request.getParameter("ghCioOrderId").trim() != null) {
					cioOrderId = (String) request.getParameter("ghCioOrderId")
							.trim();
					log.info(cioOrderId);
				} else {
				...
				CheckinOrder checkinOrder = businessService
						.findCheckinOrderByCioOrderId(cioOrderId);
				...
				object
						.put("cioGuestCatalog", checkinOrder
								.getCioGuestCatalog());
				object.put("cioGroupName", checkinOrder.getCioGroupName());
				object.put("cioCause", checkinOrder.getCioCause());
				object.put("cioInDateTime", String.valueOf(checkinOrder
						.getCioInDateTime()));
				object.put("cioPreOutDateTime", String.valueOf(checkinOrder
						.getCioPreOutDateTime()));
				object.put("cioPrctOutDateTime", String.valueOf(checkinOrder
						.getCioPrctOutDateTime()));
				object
						.put("cioPaymentModel", checkinOrder
								.getCioPaymentModel());
				...
				Page page = new Page(Integer.parseInt(start), Integer
						.parseInt(limit));
				...
				JSONArray jsonItems = new JSONArray();
				List<Balancement> balancementList = businessService
						.queryHistoryInformation(page, gtId);
				...
				while (i.hasNext()) {
					...
					CheckinOrder checkinOrder = businessService
							.findCheckinOrderByCioId(cioId);
					JSONObject object = new JSONObject();
					object.put("cioId", checkinOrder.getCioId());
					object.put("cioGuestName", checkinOrder.getCioGuestName());
					object.put("cioManNumber", checkinOrder.getCioManNumber());
					object.put("cioGuestCatalog", checkinOrder
							.getCioGuestCatalog());
					object.put("cioGroupName", checkinOrder.getCioGroupName());
					object.put("cioCause", checkinOrder.getCioCause());
					object.put("cioInDateTime", String.valueOf(checkinOrder
					...
					object.put("cioPreOutDateTime", String.valueOf(checkinOrder
							.getCioPreOutDateTime()));
					object.put("cioPrctOutDateTime", String
							.valueOf(checkinOrder.getCioPrctOutDateTime()));
					object.put("cioPaymentModel", checkinOrder
							.getCioPaymentModel());
					object
							.put("cioIsReservId", checkinOrder
									.getCioIsReservId());
					object.put("cioOrderId", checkinOrder.getCioOrderId());
					object.put("cioTotalRate", checkinOrder.getCioTotalRate());
					object.put("cioOperator", checkinOrder.getCioOperator());
	...
	public ModelAndView listAllHistoryInformation(HttpServletRequest request,
		...
		JSONArray jsonItems = new JSONArray();
		List<GuestHistory> guestHistory = businessService
				.findAllGuestHistory(page);
		...
		while (i.hasNext()) {
			...
			log.info(bmId);
			Balancement balancement = businessService
					.findBalancementInformationByBmId(bmId);
			log.info("checkinOrderId" + balancement.getBmCheckinOrderId());
			CheckinOrder checkinOrder = businessService
					.findCheckinOrderByCioId(balancement.getBmCheckinOrderId());
			Guest guest = businessService.findGuestByGtId(historyData
					.getGhGuestId());
			...
			row.put("ghPrctOutDateTime", String.valueOf(balancement
					.getBmCreateTime()));
			row.put("ghCioState", balancement.getBmType());
			row
					.put("ghPaidMoney", String.valueOf(balancement
							.getBmTotalRate()));
