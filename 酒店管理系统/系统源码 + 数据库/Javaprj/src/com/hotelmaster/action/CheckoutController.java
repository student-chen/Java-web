...
public class CheckoutController extends MultiActionController {
	...
	public ModelAndView checkout(HttpServletRequest request,
		...
		if (request.getParameter("cioGuestCardId") != null) { // 判断客户证件号码是否为空
			cioGuestCardId = (String) request.getParameter("cioGuestCardId")
					.trim(); // 获得客户证件号码
		}
		Guest guest = businessService.findGuestByGtCardId(cioGuestCardId);// 查找客户信息
		String gtId = guest.getGtId(); // 获得客户编号
		Balancement balancement = BalancementUtil.createBalancement(request,
				gtId);// 设置结账信息
		String bmId = businessService.createBalancement(balancement); // 保存结账信息
		GuestHistory guestHistroy = BalancementUtil.createGuestHistory(request,
				bmId, gtId);// 设置客户记录信息
