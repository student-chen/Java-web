...
public class ReservOrderUtil {
	...
	public static ReservOrder createReservOrder(HttpServletRequest request)
		...
		if (request.getParameter("roGuestName") != null) { // 客人姓名
			String roGuestName = (String) request.getParameter("roGuestName")
					.trim();
			reservOrder.setRoGuestName(roGuestName);
		}
		if (request.getParameter("roTelphone") != null) { // 客人电话
			String roTelphone = (String) request.getParameter("roTelphone")
					.trim();
		...
		if (request.getParameter("roInDate") != null) { // 预计住店日期
			String roInDateTime = (String) request.getParameter("roInDate")
					.trim();
			/*
			 * String temp[]=roInDateTime.split("/"); if(temp.length>0){
		...
		if (request.getParameter("roOutDate") != null) { // 预计离店日期 设置为12:00:00
			String roPreOutDateTime = (String) request
					.getParameter("roOutDate").trim();
			/*
			 * String temp[]=roPreOutDateTime.split("/"); if(temp.length>0){
			 * roPreOutDateTime=temp[2]+"-"+temp[0]+"-"+temp[1]; }
			 */
			// roPreOutDateTime.replace("/", "-");
			roPreOutDateTime = roPreOutDateTime + " 12:00:00";
			reservOrder
					.setRoPreOutDateTime(Timestamp.valueOf(roPreOutDateTime));
		}
		if (request.getParameter("roEarliestTime") != null) { // 最早到达时间
			String roEarliestTime = (String) request.getParameter(
					"roEarliestTime").trim();
		...
		if (request.getParameter("roLatestTime") != null) { // 最晚到达时间
			String roLatestTime = (String) request.getParameter("roLatestTime")
					.trim();
		...
		if (request.getParameter("roPreAssignRoom") != null) {// 预分房 无用*
			String roPreAssignRoom = (String) request.getParameter(
					"roPreAssignRoom").trim();
		...
		if (request.getParameter("roOrderId") != null) {// 操作员session取用户
			String roOrderId = (String) request.getParameter("roOrderId")
					.trim();
		...
		if (request.getParameter("roCreateTime") != null) {// //预定单创建时间
			String roCreateTime = (String) request.getParameter("roCreateTime")
					.trim();
			reservOrder.setRoCreateTime(Timestamp.valueOf(roCreateTime));
		} else {
			Calendar tCal = Calendar.getInstance();
			reservOrder
					.setRoCreateTime(new Timestamp(tCal.getTime().getTime()));
		}
	...
	public static ReservOrder createReservFromJson(JSONObject jsonObject)
			throws Exception {
		ReservOrder reservOrder = new ReservOrder();
		reservOrder.setRoCreateTime(Timestamp.valueOf(jsonObject
				.getString("roCreateTime")));
		reservOrder.setRoEarliestTime(Time.valueOf(jsonObject
				.getString("roEarliestTime")));
		reservOrder.setRoEmail(jsonObject.getString("roEmail"));
		reservOrder.setRoFax(jsonObject.getString("roFax"));
		// reservOrder.setRoGroupName(roGroupName);
		reservOrder.setRoGuestCardCatalog(jsonObject
				.getString("roGuestCardCatalog"));
		...
		reservOrder.setRoInDateTime(Timestamp.valueOf(jsonObject
				.getString("roInDateTime")));
		reservOrder.setRoLatestTime(Time.valueOf(jsonObject
				.getString("roLatestTime")));

		reservOrder.setRoOperator(jsonObject.getString("roOperator"));
		reservOrder.setRoOrderId(jsonObject.getString("roOrderId"));

		reservOrder.setRoPaidMoney(new BigDecimal(jsonObject
				.getString("roPaidMoney")));
		...
		reservOrder.setRoPreOutDateTime(Timestamp.valueOf(jsonObject
				.getString("roPreOutDateTime")));
		...
		reservOrder.setRoTotalRate(new BigDecimal(jsonObject
				.getString("roTotalRate")));

		return reservOrder;
	}

	// 22
	public static ReservOrder updateReservOrder(ReservOrder oldOrder,
			ReservOrder newOrder) {
		...
		if (!oldOrder.getRoGuestCardCatalog().equals(
				newOrder.getRoGuestCardCatalog())) {
		...
		//
		if (!oldOrder.getRoPreAssignRoom()
				.equals(newOrder.getRoPreAssignRoom())) {
			oldOrder.setRoPreAssignRoom(newOrder.getRoPreAssignRoom());
		}
		if (!oldOrder.getRoPreOutDateTime().equals(
				newOrder.getRoPreOutDateTime())) {
