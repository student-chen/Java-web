...
public class BalancementUtil {
	...
	public static Balancement createBalancement(HttpServletRequest request,
			String gtId) throws Exception {
		Balancement balancement = new Balancement();
		/*
		 * cioId=402881e41a1e00d6011a1e0c03df0002
		 * &bmOperator=FoGhost&bmOrderId=CIO00000001
		 * &bmPaymentModel=%E7%8E%B0%E9%87%91 &bmPaidMoney=0&bmReceivMoeny=0
		 * &bmType=%E7%BB%93%E5%8D%95 &bmTotalRate=320 &bmRemain=0&bmRemark=
		 */
		if (request.getParameter("bmPaymentModel") != null) { // 支付类型
			String bmPaymentModel = (String) request.getParameter(
					"bmPaymentModel").trim();
			balancement.setBmPaymentModel(bmPaymentModel);
		}
		if (request.getParameter("bmPaidMoney") != null) { // 已付押金
			String bmPaidMoney = (String) request.getParameter("bmPaidMoney")
					.trim();
			balancement.setBmPaidMoney(new BigDecimal(bmPaidMoney));
		}
		if (request.getParameter("bmReceivMoeny") != null) { // 续收金额
			String bmReceivMoeny = (String) request.getParameter(
					"bmReceivMoeny").trim();
		...
		if (request.getParameter("bmTotalRate") != null) { // 应收金额
			String bmTotalRate = (String) request.getParameter("bmTotalRate")
					.trim();
		...
		if (request.getParameter("bmOperator") != null) { // 营业员
			String bmOperator = (String) request.getParameter("bmOperator")
					.trim();
			balancement.setBmOperator(bmOperator);
		}
		if (request.getParameter("cioId") != null) { // 登记单PKId
			String bmCheckinOrderId = (String) request.getParameter("cioId")
					.trim();
	...
	public static GuestHistory createGuestHistory(HttpServletRequest request,
			String bmId, String gtId) throws Exception {
