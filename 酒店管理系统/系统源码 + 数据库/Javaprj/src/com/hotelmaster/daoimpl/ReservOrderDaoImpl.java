...
public class ReservOrderDaoImpl extends GenericHibernateDao<ReservOrder>
		implements ReservOrderDao {
	...
	public boolean create(ReservOrder reservOrder,
		...
		for (ReservItem item : reservItemList) {
			 ...
			 * item.getRoom().setRmState(item.getRoom().STATE_RENTED); }else{
			 * return false; }
			 */
	...
	public List<ReservItem> findReservItemByOrder(ReservOrder reservOrder) {
		List<ReservItem> reservItemList = hibernateTemplate.find(
				"from ReservItem reservItem where reservItem.reservOrder=?",
	...
	public void deleteReservOrder(ReservOrder reservOrder,
		...
		/*
		 * for(ReservItem item:reservItemList){ hibernateTemplate.delete(item);
		 * }
		 */
	...
	public void updateReservOrder(ReservOrder reservOrder,
			List<ReservItem> reservItemList) {
		/*
		 * hibernateTemplate.saveOrUpdate(reservOrder); for(ReservItem
		 * item:reservOrder.getReservItems()){ hibernateTemplate.delete(item); }
	...
	public ReservOrder findReservOrderByRmId(String rmId) {
		List<ReservItem> reservItemList = hibernateTemplate
				.find(
						"from ReservItem as item where item.room.rmId=? "
								+ "and item.rimState in ('预定中','已确认') order by item.rimInDateTime desc",
						rmId);
