...
public class BalancementDaoImpl extends GenericHibernateDao<Balancement>
		implements BalancementDao {
	...
	public String createBalancement(Balancement balancement) {
		...
		Timestamp curDate = new Timestamp(tCal.getTime().getTime());// 获得系统当前时间
		balancement.setBmCreateTime(curDate);// 设置结账时间
		String checkinOrderId = balancement.getBmCheckinOrderId();// 获得入住登记订单编号
	...
	public Balancement findBalancementInformationByBmId(String bmId) {
		List<Balancement> balancement = hibernateTemplate
				.find("from Balancement where bmId='" + bmId + "'");
