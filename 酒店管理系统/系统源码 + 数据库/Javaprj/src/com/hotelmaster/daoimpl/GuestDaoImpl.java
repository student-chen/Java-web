...
public class GuestDaoImpl extends GenericHibernateDao<Guest> implements
		GuestDao {
	...
	public List<Guest> findAll(Page page) {
		...
		return queryForList("select count(*) from Guest", "from Guest", null,
				page);
	}

	public long getGuestTotalCount() {
		// TODO Auto-generated method stub
		return (Long) queryForObject("select count(*) from Guest", null);
	}
	...
	public boolean delGuest(Guest guest) {
		...
		delGuest = guest;
		Long count = (Long) queryForObject(
				"select count(*) from Guest where gtId=?", new Object[] { guest
						.getGtId() });
	...
	public Guest findGuestByGtCardId(String gtCardId) {
		// TODO Auto-generated method stub
		List<Guest> guest = hibernateTemplate
				.find("from Guest where gtCardId='" + gtCardId + "'");
		if (guest.size() != 0) {
			return guest.get(0);
		} else {
			return null;
	...
	public Guest queryGuestInfo(String value, JSONArray fields) {
		/*
		 * Page page = new Page(0); String sql = "from Guest where"; for(int
		 * i=0;i<fields.size();i++){ sql = sql+"?=?"; }//like 'from guest where
		 * gtId = ? or' no finish List<Guest> guest= queryForList(
		 * "select count(*) from Guest" ,"from Guest where gtId = ?" ,new
		 * Object[] {fields.get(0),value},page );
		 */
		List<Guest> guest = hibernateTemplate.find("from Guest where gtId='"
				+ value + "'");
		if (guest.size() != 0) {
			return guest.get(0);
		} else {
