...
public class RoomDaoImpl extends GenericHibernateDao<Room> implements RoomDao {
	...
	public boolean addNewRoom(Room room) {
		...
		Long count = (Long) queryForObject(
				"select count(*) from Room where rmId=?", new Object[] { id });
	...
	/*
	 * public boolean updateRoom(Room room) { // TODO Auto-generated method stub
	 * String id = room.getRmId(); Long count=(Long)queryForObject(
	 * "select count(*) from Room where rmId=?" ,new Object[] {id});
	 * if(count.longValue()==0){ log.info("The data wasn't exist in database");
	 * return false; } else{ hibernateTemplate.update(room); return true; } }
	 */

	public List<Room> findAll(Page page) {
		return queryForList("select count(*) from Room", "from Room", null,
				page);
	}

	public Long queryRoomCount() {
		return (Long) queryForObject("select count(*) from Room", null);
	}
	...
	public boolean deleteRoom(Room room) {
		...
		delRoom = room;
		Long count = (Long) queryForObject(
				"select count(*) from Room where rmId=?", new Object[] { room
						.getRmId() });
		if (count.longValue() == 0) {
			return false;
		} else {
	...
	public Room findById(String rmId) {
		List rooms = hibernateTemplate.find("from Room as rm where rm.rmId='"
				+ rmId + "'");
		...
		if (rmCatalog.equals("全部")) {
			return queryForList(
					...
					"from Room as room where not (room.rmId) in ("
							...
							+ "select reservItem.room from ReservItem reservItem where "
							+ "reservItem.rimInDateTime>=? or reservItem.rimOutDateTime<=?) "
							+ "and room.rmAvailable=true", new Object[] {
							fromDate, toDate, fromDate, toDate }, page);
		}
		return queryForList(
				"select count(*)"
						+ "from Room as room where not (room.rmId) in ("
	...
	public Long getAvailCount(Timestamp fromDate, Timestamp toDate,
		...
		String selectCount = "select count(*)"
				...
				+ "and room.rmAvailable=true";
		Long count = (Long) queryForObject(selectCount, new Object[] {
				fromDate, toDate, rmCatalog, fromDate, toDate, rmCatalog });
		return count;
	}
