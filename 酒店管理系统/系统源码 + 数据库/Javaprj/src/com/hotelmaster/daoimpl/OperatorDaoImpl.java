...
public class OperatorDaoImpl extends GenericHibernateDao<Operator> implements
		OperatorDao {
	...
	public Operator findOperatorByOpUserName(String opUserName) {
		List operator = hibernateTemplate
				.find("from Operator where opUserName='" + opUserName + "'");
		if (operator.size() != 0) {
