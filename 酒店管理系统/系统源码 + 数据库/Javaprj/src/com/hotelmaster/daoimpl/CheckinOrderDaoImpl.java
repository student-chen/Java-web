...
public class CheckinOrderDaoImpl extends GenericHibernateDao<CheckinOrder>
		implements CheckinOrderDao {
	...
	public boolean create(CheckinOrder checkinOrder, // 创建登记信息
		...
		for (CheckinItem item : checkinItemList) { // 遍历入住登记信息列表
			...
			item.setCimInDatetime(checkinOrder.getCioInDateTime());// 设置入住时间
			item.setCimOutDatetime(checkinOrder.getCioPreOutDateTime());// 设置离开时间
	...
	public CheckinOrder findCheckinOrderByCioOrderId(String cioOrderId) {
		List checkinOrder = hibernateTemplate
				.find("from CheckinOrder where cioOrderId='" + cioOrderId + "'");
		return (CheckinOrder) checkinOrder.get(0);
	}

	public CheckinOrder findCheckinOrderByCioId(String cioId) {
		List checkinOrder = hibernateTemplate
				.find("from CheckinOrder where cioId='" + cioId + "'");
		return (CheckinOrder) checkinOrder.get(0);
	}

	public CheckinOrder findCheckinOrderByRmId(String rmId) {
		List<CheckinItem> checkinItemList = hibernateTemplate
				.find(
						"from CheckinItem as item where item.room.rmId=? "
								+ "and item.cimState='已入住' order by item.cimInDatetime desc",
						rmId);
		// 异常处理
