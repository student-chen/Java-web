...
function getAllByPage()
{
      $('#tb_departments').bootstrapTable({
            url: path+'sort.do?method=all',         //请求后台的URL（*）
            method: 'get',                      //请求方式（*）
            dataType:'json',
...
function edit(obj)
{
		var index = layer.open({
			...
			maxmin : true,
			area : [ '400px', '300px' ],
			content : path + "carsort/upp.jsp?id="+obj,
			skin : 'layui-layer-lan',
			shift : 4, // 0-6的动画形式，-1不开启
			end : function() {
				window.location.href=path+"carsort/carsort.jsp"; 
			}
		});
}
