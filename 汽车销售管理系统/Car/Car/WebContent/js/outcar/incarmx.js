...
function getAllByPage()
{
	  $('#tb_departments').bootstrapTable({
            ...
            onPostBody:function(){
            	if($.inArray("增加车辆出库",ids)==-1){
            		var e='<div><button type="button" class="btn btn-outline btn-default">'
            ...
            },
            {
                ...
                formatter : function(value, row, index) { 
	               	 var e="";
	               	 if(row.flag=="已出售"){
	               		e='<div class="oneinfos"><button type="button" class="btn btn-outline btn-default">'
	              			+'<span><a href="javascript:void(0)">出售详细信息</a></span></button></div>'; 
	               	 }else{
	               		e='<div class="myadd"><button type="button" class="btn btn-outline btn-default">'
	              			+'<span><a href="javascript:void(0)">出库操作</a></span></button></div>'; 
	               	 }
	               	 return e; 
                 }
            }
...
function myadd(){
	$(".myadd").click(function() {
		...
		layer.open({
			...
			maxmin : true,
			area : [ '400px', '350px' ],
			content : path + "outcar/add.jsp?id="+id,
			skin : 'layui-layer-lan',
			shift : 4, // 0-6的动画形式，-1不开启
			end : function() {
				$('#tb_departments').bootstrapTable('refresh');
