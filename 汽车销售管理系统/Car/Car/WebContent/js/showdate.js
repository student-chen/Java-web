...
var MonthText = new Array("", "一月", "二月", "三月", "四月", "五月", "六月", "七月", "八月",
		"九月", "十月", "十一月", "十二月");
var Calendar_obj, Calendar_obj2;
//var WriteHead=1;
document
		.write('<iframe style="position:absolute;width:206px;display:none;z-index:1000;" name="divCalendar" id="divCalendar" frameborder="0" scrolling="no"></iframe>')
/*头部信息CSS及JS函数*/
...
function Calendar_Show(year, month, day) {
	...
	content = '<scr'
			...
			+ 'ipt><div style="border:1px solid #6A84C8;height:'
			+ (64 + 21 * Trn)
			...
			+ year
			+ '年</div><div id="SArr" onclick="ShowSelect(\'SelectYear\',\'Y\');"><font face="Webdings">&#54;</font></div>'
			+ '<div id="sMonth" onclick="ShowSelect(\'SelectMonth\',\'M\');">'
			+ MonthText[month]
			+ '</div><div id="SArr" onclick="ShowSelect(\'SelectMonth\',\'M\');"><font face="Webdings">&#54;</font></div>';
	content += '</li>'
...
function Calendar(obj, obj2) {
	...
	if (arr != null) {
		...
		var mydate = new Date();
		Sdate = mydate.getFullYear() + '-' + (mydate.getMonth() + 1) + '-'
				+ mydate.getDate();
	}
	Sdate = Sdate.split("-");
