...
<table bgcolor="#FFFFFF" width="98%" cellpadding="0" cellspacing="0"
	...
	<table cellspacing="1" cellpadding="4" width="98%" align="center"
		class="tableborder">
		<tr>
			<td colspan="2" class="header"><a href="" target=""></a>选修周排课提交页面</td>
		</tr>
		<tr>
        ...
        <td width="15%" align="center" class="altbg2"><span class="STYLE1">
          <c:forEach var="list" items="${A0161}">
		    <c:forEach var="temp" items="${list}" begin="0" >
			<a
					href="ChangeLesson2listServlet?action=findoneschemeinfo&CR_NAME=${list[0]}&T_ID=${list[1]}&L2_ID=${list[2]}&TIME_ID=${list[3]}" title="点击我进入资料修改">${temp}</a>			</c:forEach>
		  </c:forEach>
		  </span>
        </td>
        <td width="15%" align="center" class="altbg2"><span class="STYLE1">
	  ...
	  <tr>
        ...
        <td width="15%" align="center" class="altbg2"><span class="STYLE1">
          <c:forEach var="list" items="${A0162}">
		    <c:forEach var="temp" items="${list}" begin="0" >
			<a
					href="ChangeLesson2listServlet?action=findoneschemeinfo&CR_NAME=${list[0]}&T_ID=${list[1]}&L2_ID=${list[2]}&TIME_ID=${list[3]}" title="点击我进入资料修改">${temp}</a>			            </c:forEach>
		  </c:forEach></span>
        ...
        <td width="15%" align="center" class="altbg2"><span class="STYLE1">
          <c:forEach var="list" items="${A0163}">
		    <c:forEach var="temp" items="${list}" begin="0" >
			<a
					href="ChangeLesson2listServlet?action=findoneschemeinfo&CR_NAME=${list[0]}&T_ID=${list[1]}&L2_ID=${list[2]}&TIME_ID=${list[3]}" title="点击我进入资料修改">${temp}</a>			            </c:forEach>
		  </c:forEach></span>
		  ...
		  <td width="15%" align="center" class="altbg2"><span class="STYLE1">
          <c:forEach var="list" items="${A0164}">
		    <c:forEach var="temp" items="${list}" begin="0" >
			<a
		...
		<tr>
			<td colspan="2" class="header">选修周排课提交页面</td>
		</tr>
	</table>
	<br>
