...
<table bgcolor="#FFFFFF" width="98%" cellpadding="0" cellspacing="0"
	...
	<form method="post" name="register"
		action="StuUpdateServlet">

	<table cellspacing="1" cellpadding="4" width="98%" align="center"
		class="tableborder">
		<tr>
			<td colspan="2" class="header"><a href="" target=""></a>学生信息-修改(必填)</td>
		</tr>
		<tr>
			...
			<td class="altbg2"><c:forEach var="list" items="${mylist}">
				...
				<c:forEach var="temp" items="${list}" begin="0" end="0">
					<input type="text" name="sid" size="25" maxlength="25"
		...
		<tr>
			<td class="altbg1" width="21%">学号（用户名）:</td>
			<td class="altbg2"><c:forEach var="list" items="${mylist}">
		...
		<tr>
			<td class="altbg1" valign="top">注册班级:</td>
			<td class="altbg2"><select name="cname">

				<c:forEach var="list2" items="${mylist2}">

					<c:forEach var="temp2" items="${list2}" begin="0" end="0">
						<option value="${temp2}" selected="selected">
					</c:forEach>
					<c:forEach var="temp2" items="${list2}" begin="1">
					              ${temp2}</option>
					</c:forEach>
	...
	<br>
	<center><input type="submit" name="regsubmit" value="保存修改信息" onclick="return check();"></center>
	</form>
