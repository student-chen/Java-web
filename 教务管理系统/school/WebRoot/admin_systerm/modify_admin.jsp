    ...
    <c:forEach var="list" items="${mylist}">
		<tr align="center">
		    <c:forEach var="temp" items="${list}" begin="0" end="0">
				<td><a
					href="ModifyServlet?action=modify&aid=${temp}" title="点击我进入资料修改">${temp}</a></td>
			</c:forEach>
			<c:forEach var="temp" items="${list}"begin="1" end="3">
				<td>${temp}</td>
			</c:forEach>
			<c:forEach var="temp" items="${list}" begin="0" end="0">
				<td><a
					href="ModifyServlet?action=modify&aid=${temp}" title="点击我进入资料修改">修改</a></td>
			</c:forEach>
